...
class testGenerator:
    def __init__(self, n_replicas, n_twins, n_rounds, n_partitions, is_leader_faulty, partition_num_limit, n_test_cases,
            ...
            for j in range(n_partitions):
                for k in range(len(self.partitions_list[i][j])):
                    if self.partitions_list[i][j][k] <= n_replicas:
                        self.partitions_list[i][j][k] = "replica_" + \
                            str(self.partitions_list[i][j][k] - 1)
                    else:
                        self.partitions_list[i][j][k] = "replica_" + \
                            str(self.partitions_list[i][j]
                                [k] - 1 - n_replicas) + "f"
        ...
        if is_deterministic == True:
            self.partitions_list = self.partitions_list[: min(
                partition_num_limit, len(self.partitions_list))]
        else:
            self.partitions_list = random.choices(self.partitions_list, k=min(
                partition_num_limit, len(self.partitions_list)))
        ...
        for i in range(n_twins, n_replicas):
            for j in range(len(self.partitions_list)):
                partition_leader_combination.append(
                    ["replica_" + str(i), self.partitions_list[j]])

        low_partition_leader_combination = []
        for i in range(n_twins, n_replicas):
            for j in range(len(self.low_partition_list)):
                low_partition_leader_combination.append(
                    ["replica_" + str(i), self.low_partition_list[j]])
        ...
        weight_val_non_faulty = 45.0 / \
            (len(partition_leader_combination) - faulty_leader_combinations)
        weight_list = []
        ...
        low_weight_val_non_faulty = 45.0 / \
            (len(low_partition_leader_combination) - low_faulty_leader_combinations)
        low_weight_list = []
        ...
        ratio = len(partition_leader_combination) / \
            (len(partition_leader_combination) +
             len(low_partition_leader_combination))
        if is_deterministic == True:
            partition_leader_combination = partition_leader_combination[:math.ceil(
                ratio * leader_partitions_num_limit)]
            low_partition_leader_combination = low_partition_leader_combination[:math.ceil(
                (1 - ratio) * leader_partitions_num_limit)]
        else:
            partition_leader_combination = random.choices(partition_leader_combination, k=math.ceil(
                ratio * leader_partitions_num_limit), weights=weight_list)
            low_partition_leader_combination = random.choices(low_partition_leader_combination, k=math.ceil(
                (1 - ratio) * leader_partitions_num_limit), weights=low_weight_list)
        ...
        while i < n_test_cases:
            ...
            low_current_round_combination = random.choices(
                low_partition_leader_combination, k=non_quorum_rounds)
            current_round_combination = random.choices(
                partition_leader_combination, k=quorum_rounds)
            test_case_data['rounds'] = {}
            for j in range(0, non_quorum_rounds):
                test_case_data['rounds'][j + 1] = {}
                test_case_data['rounds'][j +
                                         1]['leader'] = low_current_round_combination[j][0]
                test_case_data['rounds'][j +
                                         1]['partitions'] = low_current_round_combination[j][1]
                test_case_data['rounds'][j +
                                         1]['messageType'] = random.randint(0, 2)
                test_case_data['rounds'][j +
                                         1]['failType'] = random.randint(0, 2)
                test_case_data['rounds'][j + 1]['src_to_dest'] = self.get_src_dest_combs(
                    low_current_round_combination[j])

            for j in range(non_quorum_rounds, n_rounds):
                test_case_data['rounds'][j + 1] = {}
                test_case_data['rounds'][j +
                                         1]['leader'] = current_round_combination[j - non_quorum_rounds][0]
                test_case_data['rounds'][j +
                                         1]['partitions'] = current_round_combination[j - non_quorum_rounds][1]
                test_case_data['rounds'][j +
                                         1]['messageType'] = random.randint(0, 2)
                test_case_data['rounds'][j +
                                         1]['failType'] = random.randint(0, 2)
                test_case_data['rounds'][j + 1]['src_to_dest'] = self.get_src_dest_combs(
                    current_round_combination[j - non_quorum_rounds])

            is_valid_test_case = self.is_valid_test(
                test_case_data, n_replicas, n_twins, n_rounds)
            ...
            for i in range(len(test_data['rounds'][round]['partitions'])):
                ...
                for j in range(len(test_data['rounds'][round]['partitions'][i])):
                    if leader == test_data['rounds'][round]['partitions'][i][j]:
                        partitions.append(
                            test_data['rounds'][round]['partitions'][i])
                        found = True
                        break
